...
const Home = () => {
  ...
  return (
    <section className="flex flex-col items-center justify-center min-h-screen px-6 relative">
      ...
      <motion.div className="absolute w-60 h-60 bg-purple-500 blur-[90px] opacity-50" animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} />
      <motion.div className="relative w-40 h-40 sm:w-56 sm:h-56 flex items-center justify-center mt-16" animate={{ y: [0, -10, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}>
        <object type="image/svg+xml" data="/avatar-laptop.svg" className="w-full h-full"></object>
      </motion.div>
      <motion.h2 className="text-2xl sm:text-4xl font-cursive mt-6 text-center z-10" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
        Hi, I'm Priyanshu 👋🏼
      </motion.h2>
      <h3 className="text-sm sm:text-lg font-bold tracking-wide mt-2 text-center z-10">
        <Typewriter options={{ strings: ["Frontend Engineer", "React Enthusiast", "UI/UX Passionate"], autoStart: true, loop: true }} />
      </h3>
      <motion.p className="text-center text-gray-600 dark:text-gray-300 max-w-xs sm:max-w-lg mt-4 leading-relaxed z-10" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.5 }}>
